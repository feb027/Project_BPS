...
interface MainAreaProps {
  query: string
  selectedItem: {id: number, type: 'tabel' | 'indikator', title: string} | null
  setSelectedItem: (item: {id: number, type: 'tabel' | 'indikator', title: string} | null) => void
}

export function MainArea({ query, selectedItem, setSelectedItem }: MainAreaProps) {
  const { data, isLoading, error } = useSearch(query)

  const hasData = data && (data.tabel?.length > 0 || data.indikator?.length > 0)
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 gap-4">
              ...
              {data.indikator?.map((ind: any) => (
                <div key={`ind-${ind.id}`} 
                     onClick={() => setSelectedItem({id: ind.id, type: 'indikator', title: ind.nama})}
              ...
              {data.tabel?.map((tab: any) => (
                <div key={`tab-${tab.id}`} 
                     onClick={() => setSelectedItem({id: tab.id, type: 'tabel', title: tab.judul})}
                     className="group bg-card border border-border rounded-lg p-5 hover:border-primary/50 hover:shadow-md transition-all duration-200 cursor-pointer flex flex-col gap-4 relative overflow-hidden">
